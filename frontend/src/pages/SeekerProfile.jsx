...
import { Mail, ShieldCheck, User as UserIcon, Edit2, Save, X, Key, AlertTriangle } from 'lucide-react';
...
const { user, login, logout } = useAuth();
  ...
  const avatarUrl = `https://api.dicebear.com/7.x/notionists/svg?seed=${user?.name}&backgroundColor=e2e8f0`;
    ...
    <div className="max-w-3xl mx-auto space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
        <div className="h-32 bg-gradient-to-r from-blue-500 to-indigo-600 w-full relative">
          {user?.role === 'admin' && (
            <div className="absolute top-4 right-4 bg-black/30 backdrop-blur-sm text-white px-3 py-1 rounded-full text-xs font-bold flex items-center">
              <ShieldCheck className="w-4 h-4 mr-1" /> System Admin
            </div>
          )}
        </div>
        ...
        <div className="px-6 sm:px-10 pb-10">
          <div className="relative -mt-16 mb-6">
            <img src={avatarUrl} alt="Profile" className="w-32 h-32 rounded-full border-4 border-white shadow-lg bg-white" />
          </div>
          ...
          {isEditing ? (
            <form onSubmit={handleUpdate} className="space-y-6 animate-in fade-in">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  ...
                  <input type="text" name="name" value={formData.name} onChange={handleChange} className={`w-full px-4 py-3 border ${errors.name ? 'border-red-500' : 'border-slate-200'} rounded-xl focus:ring-2 focus:ring-indigo-500 outline-none bg-slate-50`} />
                  {errors.name && <p className="text-red-500 text-xs mt-1">{errors.name}</p>}
                </div>
                <div>
                  <label className="block text-sm font-semibold text-slate-700 mb-1">Email Address</label>
                  <input type="email" name="email" value={formData.email} onChange={handleChange} className={`w-full px-4 py-3 border ${errors.email ? 'border-red-500' : 'border-slate-200'} rounded-xl focus:ring-2 focus:ring-indigo-500 outline-none bg-slate-50`} />
              ...
              <div className="pt-6 border-t border-slate-100">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    ...
                    <input type="password" name="newPassword" value={formData.newPassword} onChange={handleChange} placeholder="Leave blank to keep current" className={`w-full px-4 py-3 border ${errors.newPassword ? 'border-red-500' : 'border-slate-200'} rounded-xl focus:ring-2 focus:ring-indigo-500 outline-none bg-slate-50`} />
                    {errors.newPassword && <p className="text-red-500 text-xs mt-1">{errors.newPassword}</p>}
                  </div>
                  <div>
                    <label className="block text-sm font-semibold text-slate-700 mb-1">Confirm New Password</label>
                    <input type="password" name="confirmPassword" value={formData.confirmPassword} onChange={handleChange} placeholder="Confirm new password" className={`w-full px-4 py-3 border ${errors.confirmPassword ? 'border-red-500' : 'border-slate-200'} rounded-xl focus:ring-2 focus:ring-indigo-500 outline-none bg-slate-50`} />
            ...
            <>
              <div className="mb-8">
                ...
                <p className={`font-medium flex items-center gap-2 mt-1 capitalize ${user?.role === 'admin' ? 'text-purple-600' : 'text-slate-500'}`}>
                  <ShieldCheck className="w-4 h-4" /> {user?.role} Account
                </p>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="bg-slate-50 p-5 rounded-xl border border-slate-100">
                  <p className="text-sm text-slate-500 font-semibold mb-1">Full Name</p>
